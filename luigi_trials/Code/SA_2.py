...
output_dir = "/cfs/earth/scratch/paleslui/OHPC/Output/Trial_4"
...
def simulated_annealing_parallel(
    ...
    for iteration in range(max_iterations):
        ...
        # Track MSE progression
        mse_progression.append(best_objective)
    ...
    # Save MSE progression to a file
    mse_progress_file = os.path.join(output_dir, "mse_progress.txt")
    with open(mse_progress_file, "w") as f:
        for i, mse in enumerate(mse_progression):
            f.write(f"Iteration {i}: MSE = {mse:.4f}\n")
    print(f"MSE progression saved to {mse_progress_file}")

    # Plot MSE progression
    mse_plot_file = os.path.join(output_dir, "mse_progression.png")
    plt.figure(figsize=(10, 6))
    plt.plot(mse_progression, label="MSE Progression", color="blue", linewidth=1.5)
    plt.title("MSE Progression Over Iterations", fontsize=14)
    plt.xlabel("Iteration", fontsize=12)
    plt.ylabel("MSE", fontsize=12)
    plt.grid(alpha=0.5)
    plt.legend()
    plt.savefig(mse_plot_file, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"MSE progression plot saved to {mse_plot_file}")
...
results_file_path = os.path.join(output_dir, "results_2.txt")
...
# Visualize model fit
predictions = solar_cycle_model(optimized_params, dataset['Time'].values)
model_fit_file = os.path.join(output_dir, "model_fit.png")
plt.figure(figsize=(12, 6))
plt.plot(dataset['Time'], dataset['SN'], label="Observed SN", color="blue", linewidth=1)
plt.plot(dataset['Time'], predictions, label="Predicted SN", color="red", linestyle="--", linewidth=1)
plt.title("Observed vs Predicted Sunspot Numbers", fontsize=14)
plt.xlabel("Time (Year)", fontsize=12)
plt.ylabel("Sunspot Number (SN)", fontsize=12)
plt.legend()
plt.grid(alpha=0.5)
plt.savefig(model_fit_file, dpi=300, bbox_inches="tight")
plt.close()
print(f"Model fit plot saved to {model_fit_file}")
