...
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=5  # 5 parallel chains per task
...
#SBATCH --array=0-24%10  # 25 tasks total, 10 concurrent
...
# Extract the T0 and sigma values for this task (skipping the header row)
...
#########################################################################################
# Step 2: Run Tuning for the Given Hyperparameters with Different Random Chains

CHAIN_JOBS=5  # Number of chains to run in parallel for each grid point

# Export T0 and SIGMA for easier reference inside Python
export T0 SIGMA

# Start multiple chains for the current hyperparameter set
for i in $(seq 1 $CHAIN_JOBS); do
    srun --exclusive --ntasks=1 python3 /cfs/earth/scratch/kieffleo/OHPC_da_vi_le_lu/code/main_script.py \
        --data-file /cfs/earth/scratch/kieffleo/OHPC_da_vi_le_lu/code/Datasets/data_Minimizers.csv \
        --output-dir "${OUTPUT_DIR}/job_${SLURM_ARRAY_TASK_ID}_chain${i}" \
        --tune \
        --n-iter 10000000 \
        --burn-in 1000000 \
        --log-interval 1000 \
        --seed $((42 + SLURM_ARRAY_TASK_ID * CHAIN_JOBS + i)) \
