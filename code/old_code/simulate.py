...
def main():
    ...
    parser.add_argument("--iterations", type=int, default=10000, help="Number of iterations for simulated annealing.")
    ...
    with open(args.output, "w") as log_file:
        def log_progress(message):
            print(message, flush=True)  # Flush console output immediately
            log_file.write(message + "\n")
            log_file.flush()  # Flush log file output immediately

        log_progress("Starting script...")

        # Load and preprocess data
        log_progress("Loading and preprocessing data...")
        ...
        log_progress("Data loaded and preprocessed successfully.")
    ...
    # Plot results
    log_progress("Plotting results...")
    plot_solar_model(t_obs, y_obs, optimized_params[-1])
    log_progress("Results plotted successfully.")
